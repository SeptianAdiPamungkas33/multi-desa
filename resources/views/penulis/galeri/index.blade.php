            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    ...
                    <tr class="border-b dark:border-gray-700">
                        <td scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{$item->judul_galeri}}</td>
                        <td class="px-4 py-3">
                            <img src="{{ $item->image }}" class="w-20 h-20 object-cover">
                        </td>
                        <!-- <td class="px-4 py-3">
                            <img src="{{ asset('images/1718844338.jpg') }}" alt="{{ $item->judul_galeri }}" class="w-20 h-20 object-cover">
                        </td> -->
                        <td class="px-4 py-3 flex items-center justify-end">
                            ...
                            <div id="toggles{{$item->id}}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="toggle{{$item->id}}">
                                    <li>
                                        <a href="galeri/{{$item->id}}/edit" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                    </li>
                                    <li>
                                        <form action="galeri/{{$item->id}}" method="POST" onsubmit="return confirm('Are you sure?');">
                                            @csrf
                                            @method('DELETE')
