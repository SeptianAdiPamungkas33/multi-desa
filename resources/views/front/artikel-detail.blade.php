...
<body class="overflow-x-hidden w-full  antialiased">
    ...
    <div class="flex items-center justify-center w-full pt-0 lg:pt-12 pb-12 xl:pt-12 xl:pb-24">
        <div class="container w-full flex flex-col items-center justify-center px-12 h-full py-12">
            <div class="relative z-50 flex items-center justify-center w-full h-full">
                <img src="{{ asset($postingan->gambar) }}" class="w-6/12">
            </div>
            <div class="flex flex-col items-center py-2 w-full lg:items-start lg:w-full lg:py-6 xl:py-6 lg:text-left">
                <h1 class="relative mb-4 text-2xl font-black leading-tight text-gray-900 sm:text-4xl xl:py-2">{{ $postingan->judul }}</h1>
                <p class="py-1 text-base text-gray-600 sm:text-lg xl:text-xl lg:pr-20 text-yellow-500">{{ $postingan->kategori->nama }}</p>
                <p class="py-1 text-base text-gray-600 sm:text-lg xl:text-xl lg:pr-20">{!! $postingan->isi !!}</p>
            </div>
        </div>

        <!-- Slider Section -->
        <div class="w-[400px] flex flex-col justify-center items-center py-6 gap-4 pr-12">
            <div class="font-bold text-2xl">Pilih Artikel Lain</div>
            <div class="w-full h-[500px] flex-col overflow-y-auto gap-4 scrollbar-thin scrollbar-thumb-rounded scrollbar-thumb-gray-400">
                @foreach($website->postingan->where('status', 'aktif') as $postingans)
                <div class="flex-shrink-0 w-[300px] flex-wrap flex items-center p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
