...
<div class="p-4 mt-16 sm:ml-64">
    <p class="text-3xl font-bold text-blue-600 uppercase">Halaman Edit Penulis</p>
    <form action="/editor-penulis/{{$user->id}}" method="post" class="max-w-xl">
        @csrf
        @method('put')
        <div class="mb-5">
        ...
        <div>
            ...
            <select id="desa_id" name="desa_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                ...
                <option value="{{ $item->id }}" {{ isset($user) && $item->id === 1 ? 'selected' : '' }}>
                    {{ $item->nama }}
                </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="role_id" name="role_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                ...
                <option value="{{ $item->id }}" {{ isset($user) && $item->id === $user->role_id ? 'selected' : '' }}>
                    {{ $item->name }}
                </option>
                @endforeach
            </select>
